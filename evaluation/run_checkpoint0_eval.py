...
OUTPUT_DIR = "outputs/evals/checkpoint0"
...
print("Loading base model (Checkpoint 0)...")
...
for i, ex in enumerate(alpaca_eval):
    print(f"[Alpaca C0] {i+1}/{len(alpaca_eval)}")
...
with open(os.path.join(OUTPUT_DIR, "alpaca_checkpoint0_outputs.json"), "w") as f:
...
for i, ex in enumerate(json_eval):
    print(f"[JSON C0] {i+1}/{len(json_eval)}")
    pred = generate_response(ex["instruction"], ex.get("input", ""))
...
with open(os.path.join(OUTPUT_DIR, "json_checkpoint0_outputs.json"), "w") as f:
...
with open(os.path.join(OUTPUT_DIR, "json_checkpoint0_summary.json"), "w") as f:
    json.dump(json_summary, f, indent=2)

print("\n=== Checkpoint 0 Evaluation Complete ===")
print(f"Saved Alpaca outputs to: {OUTPUT_DIR}/alpaca_checkpoint0_outputs.json")
print(f"Saved JSON outputs to: {OUTPUT_DIR}/json_checkpoint0_outputs.json")
print(f"Saved JSON summary to: {OUTPUT_DIR}/json_checkpoint0_summary.json")
print("\nJSON Summary:")
